...
public class AI {
    ...
    public int[] doAI(char[] _tools,boolean _deBug, int _loopCount,boolean _attactive)
    {
        ...
        deBug = _deBug;
        tools = AIDATA.oldFormatToNew(_tools);
                ...
                for(int g = 0; g < 64; g++)
                {
                    if(colors[g] == 'y' || colors[g] == 'o')
                    {
                        ...
                        if(countTools(tools) < 6 && onlyKingCanMove(tools) && inDanger(findTool('l'),tools)) // check if the move will put the king on pat
                        {
                            ...
                            char[] kindMoves = AIDATA.doBlackMove(tools,kingLocaiton);
                            for(int j = 0; j < 64; j++)
                            {
    ...
    private boolean whiteCharIsProtect(char[] tools, int loc)
    {
        for(int i = 0; i < 64; i++)
        {
            if(isWhite(tools[i]) && AIDATA.doWhiteMove(tools,i)[loc] == 'o')
                return true;
        }
        return false;
    }
    private boolean onlyKingCanMove(char[] tools)
    {
        ...
        for(int i = 0; i < 64; i++)
        {
            if(isBlack(tools[i]))
            {
                ...
                {
                    char[] move = AIDATA.doBlackMove(tools,i);
                    for(int g = 0; g < 64; g++)
                    {
    ...
    public class priorityMove implements Cloneable{
        ...
        public boolean ifGetEatAfterMove(){return _getEatAfterMove;}
    ...
    {
        public static char[] doWhiteMove(char[] tools, int loc)
        {
            char kind = tools[loc];
            char[] movesColor;
            switch (kind) {
                case 'a': movesColor = AIDATA.whitePawnMove(tools, loc); break;
                case 'b': movesColor = AIDATA.hoursMove(tools, loc); break;
                case 'c': movesColor = AIDATA.bisMove(tools, loc); break;
                case 'd': movesColor = AIDATA.towerMove(tools, loc); break;
                case 'e': movesColor = AIDATA.queenMove(tools, loc); break;
                case 'f': movesColor = AIDATA.kingMove(tools, loc); break;
                default:movesColor = new char[64]; for(int i = 0; i < 64; i++) {movesColor[i] = 'n';} break;
            }
            return movesColor;
        }
        public static char[] doBlackMove(char[] tools, int loc)
        {
            char kind = tools[loc];
            char[] movesColor;
            switch (kind) {
                case 'g': movesColor = AIDATA.blackPawnMove(tools, loc); break;
                case 'h': movesColor = AIDATA.hoursMove(tools, loc); break;
                case 'i': movesColor = AIDATA.bisMove(tools, loc); break;
                case 'j': movesColor = AIDATA.towerMove(tools, loc); break;
                case 'k': movesColor = AIDATA.queenMove(tools, loc); break;
                case 'l': movesColor = AIDATA.kingMove(tools, loc); break;
                default:movesColor = new char[64]; for(int i = 0; i < 64; i++) {movesColor[i] = 'n';} break;
            }
            return movesColor;
        ...
        public static char[] whitePawnMove(char[] tools,int loc)
        {
            char[] colors = new char[64];
            setDor(colors, loc);
            if(!up && tools[loc - 8] == 'n')
            ...
            if(!right && !up && data.isBlack(tools[loc - 7]))
                colors[loc - 7] = 'g';
            else if(!right && !up && data.isWhite(tools[loc - 7]))
                colors[loc - 7] = 'o';
            if(!left && !up && data.isBlack(tools[loc - 9]))
                colors[loc - 9] = 'g';
            else if(!left && !up && data.isWhite(tools[loc - 9]))
                colors[loc - 9] = 'o';
            return colors;
        }
        public static char[] blackPawnMove(char[] tools, int loc)
        {
            char[] colors = new char[64];
            setDor(colors, loc);
            if(!down && tools[loc + 8] == 'n')
            ...
            if(!right && !down && data.isBlack(tools[loc + 9]))
                colors[loc + 9] = 'g';
            else if(!right && !down && tools[loc + 9] != 'n')
                colors[loc + 9] = 'o';
            if(!left && !down && data.isBlack(tools[loc + 7]))
                colors[loc + 7] = 'g';
            else if(!left && !down && tools[loc + 7] != 'n')
                colors[loc + 7] = 'o';
            return colors;
        }
        public static char[] hoursMove(char[] tools, int loc)
        {
            char[] colors = new char[64];
            setDor(colors, loc);
            if(loc > 16 && !left)
            {
                if(tools[loc - 17] == 'n')
                    colors[loc - 17] = 'y';
                else if(data.isBlack(tools[loc - 17]))
                    colors[loc - 17] = 'g';
                else
                    colors[loc - 17] = 'o';
            }
            if(loc > 15 && !right)
            {
                if(tools[loc - 15] == 'n')
                    colors[loc - 15] = 'y';
                else if(data.isBlack(tools[loc - 15]))
                    colors[loc - 15] = 'g';
                else
                    colors[loc - 15] = 'o';
            }
            if(!(loc % 8 == 0 || loc % 8 == 1) && !up)
            {
                if(tools[loc - 10] == 'n')
                    colors[loc - 10] = 'y';
                else if(data.isBlack(tools[loc - 10]))
                    colors[loc - 10] = 'g';
                else
                    colors[loc - 10] = 'o';
            }
            if(!(loc % 8 == 6 || loc % 8 == 7) && !up)
            {
                if(tools[loc - 6] == 'n')
                    colors[loc - 6] = 'y';
                else if(data.isBlack(tools[loc - 6]))
                    colors[loc - 6] = 'g';
                else
                    colors[loc - 6] = 'o';
            }
        ...
        public static char[] bisMove(char[] tools, int loc)
        {
            char[] colors = new char[64];
            setDor(colors, loc);
            for(int i = loc + 7; i < 64; i+= 7)
            {
                ...
                {
                    if(data.isWhite(tools[i])) {
                        colors[i] = 'o';
                        break;
                    }
                    else if(data.isBlack(tools[i]))
                    {
                        colors[i] = 'g';
                        break;
                    }
        ...
        public static char[] towerMove(char[] tools, int loc)
        {
            char[] colors = new char[64];
            setDor(colors, loc);
            //System.out.println(loc);
        ...
        public static char[] queenMove(char[] tools, int loc)
        {
        ...
        public static char[] kingMove(char[] tools, int loc)
        {
            char[] colors = new char[64];
            setDor(colors, loc);
            if(!up)
            {
                if(tools[loc - 8] == 'n')
                    colors[loc - 8] = 'y';
                else if(data.isBlack(tools[loc - 8]))
                    colors[loc - 8] = 'g';
                else if(tools[loc] - 8 != 'n')
                    colors[loc - 8] = 'o';
            }
            if(!right)
            {
                if(tools[loc + 1] == 'n')
                    colors[loc + 1] = 'y';
                else if(data.isBlack(tools[loc + 1]))
                    colors[loc + 1] = 'g';
                else if(tools[loc] + 1 != 'n')
                    colors[loc + 1] = 'o';
                if(!down)
                {
                    if(tools[loc + 9] == 'n')
                        colors[loc + 9] = 'y';
                    else if(data.isBlack(tools[loc + 9]))
                        colors[loc + 9] = 'g';
                    else if(tools[loc] + 9 != 'n')
                        colors[loc + 9] = 'o';
                }
                if(!up)
                {
                    if(tools[loc - 7] == 'n')
                        colors[loc - 7] = 'y';
                    else if(data.isBlack(tools[loc - 7]))
                        colors[loc - 7] = 'g';
                    else if(tools[loc] - 7 != 'n')
                        colors[loc - 7] = 'o';
                }
        ...
        public static char[] oldFormatToNew(char[] what)
        {
            char[] _tools = new char[64];
            for(int i = 0; i < 64; i++)
            {
                _tools[i] = what[i + 1];
            }
